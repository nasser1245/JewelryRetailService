<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ucMemberList.ascx.cs" Inherits="Manager_UC_ucMemberList" %>
...
<script type="text/javascript">
            ...
            $.ajax({
                ...
                success: function (msg) {
                    if (msg.d == "1") {
                        ...
                        var typepath = $(obj).parent().parent().find('img[imglock="true"]').attr("typepath");
                        if (typepath == "true.jpg")
                            typepath = "false.jpg";
                        else
                            typepath = "true.jpg";
                        $(obj).parent().parent().find('img[imglock="true"]').attr("typepath", typepath);
                        $(obj).parent().parent().find('img[imglock="true"]').attr("src", "../../Resource/PicSite/active" + typepath);
                        $(obj).parent().parent().find('img[imglock="true"]').attr("imglock", "true");
    ...
    function removeMember(obj, UserName) {
        if (confirm('آیا از حذف این کاربر اطمینان دارید؟')) {
            if (obj) {
                $.ajax({
                    type: "POST",
                    url: "MemberList.aspx/DeleteMember",
                    data: "{'UserName':'" + UserName + "'}",
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (msg) {
...
<div style="padding:5px; margin:5px;">
    ...
    <div style="margin:20px">
        <asp:Button CssClass="btn1" ID="btnAdd" runat="server" Text="افزودن مدیر" onclick="btnAdd_Click"  />
...
<div style="width:500px; margin-bottom:20px; margin-top:20px">
    <div class="titrgrid">
        <span style="width:150px; margin-right:40px">نام کاربری</span>
        <span style="width:100px; margin-right:40px">نام و نام خانوادگی</span>
        <span style="width:100px; margin-right:40px">ایمیل</span>
        <span style="width:100px; margin-right:50px">عملیات</span>
        ...
        <asp:Repeater ID="rptArticle" runat="server" >
            ...
            <ItemTemplate>
                <div class="odd">
                    ...
                    <span style="width:100px"><%# string.IsNullOrEmpty((Eval("FirstName").ToString() + Eval("LastName").ToString())) ? " --- " : ((Eval("FirstName").ToString() + Eval("LastName").ToString()).Length > 20 ? (Eval("FirstName").ToString() + Eval("LastName").ToString()).Substring(0, 20) + ".." : (Eval("FirstName").ToString() + Eval("LastName").ToString()))%></span>  
                    <span style="width:100px"><%# string.IsNullOrEmpty(Eval("Email").ToString()) ? " --- " : (Eval("Email").ToString().Length > 20 ? Eval("Email").ToString().Substring(0, 20) + ".." : Eval("Email").ToString())%></span>  
                    ...
                    <span style="width:100px">
                        <a href="MemberContent.aspx?id=<%# Eval("id") %>" title="ویرایش" ><img id="img1" alt="" src='<%#Page.ResolveClientUrl("~/Resource/PicSite/edit-icon.png")%>' class="mn_l5" width="16" height="16" /></a>
                        <a title="حذف" href="#" onclick='return removeMember(this,"<%# Eval("UserName") %>")'><img id="imgBtnDelete" alt="حذف" src='<%#Page.ResolveClientUrl("~/Resource/PicSite/delete-icon.png")%>' class="mn_l5" width="16" height="16" /></a>
                        <a title = "تغییر سطح دسترسی ها" href="MemberAccess.aspx?UserName=<%# Eval("UserName") %>"><img id="img2" alt="" src='<%#Page.ResolveClientUrl("~/Resource/PicSite/ChangeLevelAcsess.png")%>' class="mn_l5" width="16" height="16" /></a>
                        <a title='<%# Eval("Locked").ToString().ToLower() == "false" ? "غیرفعال کردن" :  "فعال کردن" %>' href="#" onclick='return LockMember(this,"<%# Eval("UserName") %>")'><img imglock="true" typepath='<%# Eval("Locked").ToString().ToLower() +  ".jpg" %>' src = '<%= Page.ResolveClientUrl("~/Resource/PicSite/active")%><%# Eval("Locked").ToString().ToLower() +  ".jpg" %>' alt="غیرفعال" class="mn_l5" width="16" height="16" /></a>
                 </span>
                 <%} %>
                 <div class="clear"></div>
                </div>
            ...
            <AlternatingItemTemplate>
                <div class="even">
                    ...
                    <span style="width:100px">
                        ...
                        <a href="MemberContent.aspx?id=<%# Eval("id") %>" title="ویرایش" ><img id="img1" alt="" src='<%#Page.ResolveClientUrl("~/Resource/PicSite/edit-icon.png")%>' class="mn_l5" width="16" height="16" /></a>
                        <a href="#" onclick='return removeMember(this,"<%# Eval("UserName") %>")'><img id="imgBtnDelete" alt="" src='<%#Page.ResolveClientUrl("~/Resource/PicSite/delete-icon.png")%>' class="mn_l5" width="16" height="16" /></a>
                        <a title = "تغییر سطح دسترسی ها" href="MemberAccess.aspx?UserName=<%# Eval("UserName") %>"><img id="img2" alt="" src='<%#Page.ResolveClientUrl("~/Resource/PicSite/ChangeLevelAcsess.png")%>' class="mn_l5" width="16" height="16" /></a>
                        <a title='<%# Eval("Locked").ToString().ToLower() == "false" ? "غیرفعال کردن" :  "فعال کردن" %>' href="#" onclick='return LockMember(this,"<%# Eval("UserName") %>")'><img imglock="true" typepath='<%# Eval("Locked").ToString().ToLower() +  ".jpg" %>' src = '<%= Page.ResolveClientUrl("~/Resource/PicSite/active")%><%# Eval("Locked").ToString().ToLower() +  ".jpg" %>' alt="غیرفعال" class="mn_l5" width="16" height="16" /></a>
                    </span>
                                     <%} %>
